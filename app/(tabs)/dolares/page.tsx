      ...
      {loading ? (
        ...
        <>
          ...
          <div className="card" style={{ borderColor: "var(--yellow)44", background: "linear-gradient(135deg, var(--surface) 0%, var(--yellow-dim) 100%)", marginBottom: 10 }}>
            <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 8 }}>Reserva total</div>
            ...
            {reservaEnARS && (
              <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 8 }}>
                ≈ {formatARS(reservaEnARS)} · {tipoCambioRef} ${cotizacionActual?.toLocaleString("es-AR")}
              </div>
            )}
            {gananciaARS !== null && (
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid var(--border)", display: "flex", justifyContent: "space-between" }}>
                <div>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 700 }}>${costoPromedio.toLocaleString("es-AR", { maximumFractionDigits: 0 })}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 700, color: gananciaARS >= 0 ? "var(--green)" : "var(--red)" }}>
                    {gananciaARS >= 0 ? "+" : ""}{formatARS(gananciaARS)}
          ...
          <div className="card" style={{ borderColor: "var(--accent)33", marginBottom: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <div>
                <div className="label" style={{ marginBottom: 4 }}>Meta acumulada</div>
                <div style={{ fontSize: 20, fontWeight: 700, fontFamily: "var(--font-mono)" }}>
                  U$D {totalUSD.toFixed(0)} <span style={{ fontSize: 13, color: "var(--muted)", fontWeight: 400, fontFamily: "var(--font)" }}>/ {metaUSD}</span>
                </div>
              </div>
              ...
              <div key={m.id} className="row">
                ...
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "var(--yellow)", fontFamily: "var(--font-mono)" }}>
                    +{m.cantidadUSD?.toFixed(2)} USD
                  </div>
                  <div style={{ fontSize: 10, color: "var(--muted)" }}>{formatARS(m.monto)}</div>
                </div>
              </div>
            ))}
          </div>
